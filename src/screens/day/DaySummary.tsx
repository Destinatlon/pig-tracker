  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.surface, borderBottomColor: colors.divider }]}>
      <View style={styles.textBlock}>
        <Text style={[styles.calories, { color: colors.textPrimary }]} accessibilityLabel={t('day.summaryA11y', { consumed: formatCalories(totals.calories), target: goal ? formatCalories(goal.calories) : '—' }) + (over ? t('day.summaryOverA11y') : '')}>
          <Text style={over ? { color: colors.warning, textDecorationLine: 'underline' } : undefined}>{formatCalories(totals.calories)}</Text>
          <Text style={{ color: colors.textSecondary }}> / {goal ? formatCalories(goal.calories) : '—'} {t('common.kcal')}</Text>
